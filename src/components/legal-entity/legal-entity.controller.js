...
import {LegalEntityIdentifier, LegalEntity} from './legal-entity.model';
...
export default class LegalEntityController {
  ...
  deleteLegalEntity($event) {
    ...
    this.$mdDialog.show(confirm).then(() => {
      ...
      self.legalEntityService.deleteLegalEntity(self.selected._id)
        .then(() => self.legalEntities.splice(self.selectedIndex, 1));
    });
  }
  ...
  saveLegalEntity($event) {
    ...
    if (this.selected != null && this.selected._id != null) {
      this.legalEntityService.updateLegalEntity(this.selected).then(() => {
        // self.$mdDialog.show(
        //     self.$mdDialog
        //         .alert()
      ...
      this.legalEntityService.createLegalEntity(this.selected).then(() => {
        // self.$mdDialog.show(
        //     self.$mdDialog
        //         .alert()
        //         .clickOutsideToClose(true)
  ...
  showContactPersonDiag(person, $event) {
    this.$mdDialog.show({
      ...
      controllerAs: '_ctrl',
      templateUrl: './components/legal-entity/contact-person.template.html',
