      ...
      <div slot='split-pane-1'>
       ...
       <div slot='split-pane-2' class='pane'>
        <template v-if='appRecords && appRecords.length'>
          ...
          <vue-table id='ingredients' :title='$t("INGREDIENTS")' :items='model.ingredients.ingredient' :headers='["quantity", {name: "unit", url: "picklist"}]' :displayHeader='displayTranslation' @select='showDialog("ingredients", model.ingredients.ingredient[$event], $event)' addable @add='addItem("ingredients")'></vue-table>
        </template>
        <template v-else>
          {{$t('noitems')}}
        </template>
