<template>
  <div style='position: relative;'>
    ...
    <main class='pane'>
      ...
      <vue-table v-if='dossiers.length && dossier' id='submissions' :title='$tc("submission", 2)' addable @add='addSubmission' @select='selectSubmission($event)' :items='mappedSubmissions' :headers='["submissiontitle", "submissionnumber", {key: "_state", name: "SUBMISSION_STATUS"}, {key: "packagetype", name: "PACKAGE_TYPE"}, {key: "_created", name: "created"}, {key: "_lastMod", name: "lastmodified"}]' :displayHeader ='$t' editable viewable @action='handleAction($event, dossier, "submission")' :isDeletable='s => s.deletable' :isEditable='s => s.editable' :isViewable='s => s.viewable' initialSortBy='submissionnumber' :initialDesc='true'></vue-table>
