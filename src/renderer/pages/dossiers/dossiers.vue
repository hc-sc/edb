<template>
  <div style='position: relative;'>
    ...
    <main class='pane'>
      ...
      <vue-table id='dossiers' :title='$t("dossiers")' addable :items='mappedDossiers' :headers='["dossierdescriptiontitle", "productname", {key: "_state", name: "DOSSIER_STATUS"}, {key: "_created", name: "created"}, {key: "_lastMod", name: "lastmodified"}]' :displayHeader='$t' initialSortBy='_lastMod' :initialDesc='true' @select='selectDossier($event)' editable @add='addDossier' @action='handleAction($event, dossiers, "dossier")' :isEditable='e => e.editable' :isDeletable='e => e.deletable'></vue-table>

      <vue-table v-if='dossiers.length && dossier' id='submissions' :title='$t("submissions")' addable @add='addSubmission' @select='selectSubmission($event)' :items='mappedSubmissions' :headers='["submissiontitle", "submissionnumber", {key: "_state", name: "SUBMISSION_STATUS"}, {key: "packagetype", name: "PACKAGE_TYPE"}, {key: "_created", name: "created"}, {key: "_lastMod", name: "lastmodified"}]' :displayHeader ='$t' editable viewable @action='handleAction($event, dossier, "submission")' :isDeletable='s => s.deletable' :isEditable='s => s.editable' :isViewable='s => s.viewable' initialSortBy='submissionnumber' :initialDesc='true'></vue-table>

      <template v-else>
        <p v-if='!dossiers.length'>{{$t('noitems')}}</p>
        <p v-else-if='!dossier'>{{$t('NO_DOSSIER_SELECTED')}}</p>
      </template>
    </main>
  </div>
</template>
